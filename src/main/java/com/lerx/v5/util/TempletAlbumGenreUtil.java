...
public class TempletAlbumGenreUtil {

	/*
	 * 根据tag字符串得到当前模板
	 */
	public static TempletSubElement elInitByTagStr(TempletAlbumGenreMain templetMain, String tagStr) {
	...
	// 通用模块代码替换
	public static String mainCodeReplace(String html, TempletAlbumGenreMain template) {
	...
	//得到逝去的时间说明
	public static String statTimesLosted(TempletAlbumGenreMain template,long datetime) {
	 ...
	 * 获得逝去的时间标签库
	 */
	public static DatetimeSpanSuffixTxt sundriesTimesLostedTag(TempletAlbumGenreMain template) {
	 ...
	 * 解析标签
	 */
	public static String sundriesTag(TempletAlbumGenreMain template,
			String station) {
		String tagVal="";
	...
	public static String sundriesTag(ResourceBundleMessageSource messageSource,TempletAlbumGenreMain template,
			String station) {
	...
	private static FindedDataAnalyzeResult lfSetup(FindedDataAnalyzeResult fdar, TempletAlbumGenreMain main,
	 ...
	 */
	public static FindedDataAnalyzeResult fmt(FindedDataAnalyzeResult fdar, TempletAlbumGenreMain main,
			TempletSubElement el) {
	...
	public static String tagToNavs(String html, EnvirSet es, FindedDataAnalyzeResult fdar) {
		...
		TempletAlbumGenreMain template=es.getTempletAlbumGenreMainDaoImpl().findDef();
		DataShowParams dsp = fdar.getDsp();
		boolean gather=false;
		
		String lf = dsp.getLoopFormatStr();
		String tmp, tmpAll = "";
		String imgHtmlTemplet=TempletAlbumGenreUtil.sundriesTag(template, "imgHtmlTemplet");
		boolean asc;
		if (dsp.getOrder() == 0) {
			asc = false;
		} else {
		...
		if (dsp.getDataSource().equalsIgnoreCase("articles_from_navs_sub")) {		//在本方法内同步处理指定顺序号的栏目数据
			...
			String noPassedTag=TempletAlbumGenreUtil.sundriesTag(template, "nopass");
			...
			if (listAgn!=null && listAgn.size()>0) {
				for (ArticleGroupNearby agn : listAgn) {
					...
					propTag=TempletAlbumGenreUtil.sundriesTag(template, "propTag" + agn.getProp());
...
//		System.out.println("fdar.getWholeTag():"+fdar.getWholeTag());
		...
		if (dsp.getDataSource().equalsIgnoreCase("articles")) {
			if (dsp.getSingle() == 1) {
				...
				if (dsp.getSpare1()==1 && fdar.getDsp().getSubLoopFormatStr()!=null && !fdar.getDsp().getSubLoopFormatStr().trim().equals("")) {		//如果是自动首页全部栏目文章
					...
					String isPassedTag=TempletAlbumGenreUtil.sundriesTag(template, "passed");
					String noPassedTag=TempletAlbumGenreUtil.sundriesTag(template, "nopass");
					for (Object obj : rs.getList()) {
						step++;
						Article article = (Article) obj;
	...
	public static String tagToEnvir(String html, EnvirSet is) {
		...
		AlbumGenre ag=is.getAlbumGenreDaoImpl().findByID(is.getGid());
		TempletAlbumGenreMain template;
		if (ag==null) {
			template=null;
		}else {
	...
	public static TempletAlbumGenreMain copy(TempletAlbumGenreMain oldTemplate,TempletAlbumGenreMain newTemplate){
	...
	public static TempletAlbumGenreMain copy(TempletAlbumGenreMain oldTemplate,String newTitle){
		TempletAlbumGenreMain newTemplate=new TempletAlbumGenreMain();
		newTemplate.setAuthor(oldTemplate.getAuthor());
		newTemplate.setResFolder(oldTemplate.getResFolder());
		newTemplate.setCustomFormatCode1(oldTemplate.getCustomFormatCode1());
		newTemplate.setCustomFormatCode2(oldTemplate.getCustomFormatCode2());
	 ...
	 * 查询并返回一个简单模板列表
	 */
	public static List<TempletSimple> queryAllSimple(ITempletAlbumGenreMainDao templetAlbumGenreMainDaoImpl){
		List<TempletAlbumGenreMain> list=templetAlbumGenreMainDaoImpl.queryAll();
		List<TempletSimple> listSimple= new ArrayList<TempletSimple>();
		for (TempletAlbumGenreMain templet:list) {
			if (templet.isState()) {
