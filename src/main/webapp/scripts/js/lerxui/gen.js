...
function modifyAttaTitle(path,aid,tmpID,divID,attaID){
	var w=parentW();
	w.layer.prompt({
		  formType: 0,
		  value: '',
		  ...
		  area: ['800px', '350px'] //自定义文本域宽高
			...
			var uri=path+'/action_atta/title/'+attaID+'?title='+encodel(value);
			w.layer.close(index);
			$.ajax({
				url: uri,
				type: 'post',
				success: function(data) {
					...
					if (data==0){
						...
						artattasReload(path,aid,tmpID,divID);
						w.layer.msg('修改成功！');
...
function removeArtAtta(path,aid,tmpID,divID,attaID){
	
	var w=parentW();
	w.layer.confirm('您确定删除该附件 吗？ 删除后将不可恢复！',{title:'警告'}, function(index){
		  
		var uri=path+'/action_atta/del/'+attaID;
...
function artattasReload(path,aid,tmpID,divID){
	var uri=path+'/action_atta/reloadByAid/'+aid+"/"+tmpID;
    $.ajax({
			url: uri,
			type: 'post',
			async: false,
			success: function(data) {
				...
				for(var i=0,l=data.length;i<l;i++){
					...
					if (data.length>1){
						tmp='<tr><td>附件'+(i+1)+':</td><td><span class="layui-hide" id="attaID">'+attaID+'</span><a target="_blank" title="'+title+'" href="'+path+'/'+item.url+'">'+title+'</a></td><td ><a title="删除" href="javascript:removeArtAtta(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');"><i class="layui-icon layui-icon-delete"></i></a> 　<a title="修改名称" href="javascript:modifyAttaTitle(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');"><i class="layui-icon layui-icon-edit"></i></a></td></tr>';
						//tmp='<li><span class="layui-hide" id="attaID">'+attaID+'</span><span class="glyphicon glyphicon-paperclip">附件'+(i+1)+'：</span>　<a title="'+title+'" href="'+path+'/'+item.url+'">'+title+'</a> <a href="javascript:removeArtAtta(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');">移除</a></li>';
					}else{
						tmp='<tr><td colspan=2><span class="layui-hide" id="attaID">'+attaID+'</span><a target="_blank" title="'+title+'" href="'+path+'/'+item.url+'">'+title+'</a></td><td ><a title="删除" href="javascript:removeArtAtta(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');"><i class="layui-icon layui-icon-delete"></i></a> 　<a title="修改名称" href="javascript:modifyAttaTitle(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');"><i class="layui-icon layui-icon-edit"></i></a></td></tr>';
						//tmp='<li><span class="layui-hide" id="attaID">'+attaID+'</span><span class="glyphicon glyphicon-paperclip"></span>　<a title="'+title+'" href="'+path+'/'+item.url+'">'+title+'</a> <a href="javascript:removeArtAtta(\''+path+'\','+aid+','+tmpID+',\''+divID+'\','+attaID+');">移除</a></li>';
					}
					//alert('tmp:'+tmp);
					endhtml += tmp;
				}
...
function commadd(path,bid) {
	...
	//alert("document.body.offsetWidth:"+document.body.offsetWidth);
		...
		title: '增加评论',
		    area : [lw+'px' , lh+'px'],
		    content: path+'/jsp/comm/add?bid='+bid,
		    success: function(layero, index){
		    },
		    end: function(){
		    	location.reload();
						...
						yes: function(index){
							...
							//注册
								...
								w.layer.prompt({
									...
									//发送验证码
									uri =path+'/action_captcha/send?lookup=1&target=' + value;
									$.ajax({
										url : uri,
										type : 'post',
										success : function(data) {
...
function artView(path,aid){
	$.ajax({
		url:path+'/show_portal/artViewShow/'+aid+"?url="+encodeURIComponent(window.location.href)+"&referer="+encodeURIComponent(document.referrer),
		type: 'post',
		success:function(data){
			$("#viewsarea").html(data);
...
function navView(path,gid){
	$.ajax({
		url:path+'/show_portal/navViewShow/'+gid+"?url="+encodeURI(window.location.href)+"&referer="+encodeURI(document.referrer),
		type: 'post',
		success:function(data){
			//$("#viewsarea").html(data);
...
function indexView(path){
	$.ajax({
		url:path+'/show_portal/indexViewShow'+"?url="+encodeURI(window.location.href)+"&referer="+encodeURI(document.referrer),
		type: 'post',
		success:function(data){
			//$("#viewsarea").html(data);
