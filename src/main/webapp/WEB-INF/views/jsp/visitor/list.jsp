...
<blockquote class="layui-elem-quote layui-quote-nm"> >> <i class="layui-icon">&#xe60e;</i> <c:if test="${requestScope.portal }">网站</c:if>来访信息列表　　　　 总访问量：${vbook.viewsTotal }　|　 总IP量：${vbook.ipTotal } 　　(今日：${vr.currDay} 　本周：${vr.currWeek } 　本月：${vr.currMonth } 　本季度：${vr.currQuarter } 　本年：${vr.currYear })</blockquote>
   ...
   <tbody>
    <c:forEach items="${requestScope.rs.list }" var="vip" varStatus="st">
					...
					<tr>
						...
						<td>${vip.ip }<c:if test="${vip.totalIn6Hours>1 }"><a href="javascript:details(${vip.id});"> (${vip.totalIn6Hours}) </a></c:if></td>
...
function details(id){
	var w=parentW();
	var uri='${pageContext.request.contextPath}/action_visitor/details/'+id;
