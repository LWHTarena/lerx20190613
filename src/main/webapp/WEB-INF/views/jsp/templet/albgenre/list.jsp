...
<table class="layui-table" lay-filter="demo">
				...
				<script lang='javascript' type='text/javascript'> 
					...
					<tr>
						...
						<td><a title="编辑" href="javascript:details('${templet.name }',${templet.id })"> <i class="layui-icon">&#xe642;</i> </a>　<a title="删除" href="javascript:del('${templet.name }',${templet.id })"> <i class="layui-icon">&#xe640;</i> </a>　<a title="复制" href="javascript:copy(${templet.id })"> <i class="layui-icon">&#xe630;</i> </a>　<a  title="下载"  href="${pageContext.request.contextPath}/action_albgenre_templet/download/${templet.id }"> <i class="layui-icon">&#xe601;</i> </a>　<a id="${templet.uuid }_report" title="发布更新到模板市场" href="javascript:report('${templet.name }',${templet.id })"> <i class="layui-icon layui-icon-upload-drag"></i> </a> 　<span class="clouds" id="${templet.uuid }_clouds">|</span> 　<a class="clouds" id="${templet.uuid }_freechg" href="javascript:freechg('${templet.name }','${templet.uuid }')"> <i class="layui-icon"></i> </a>　<a class="clouds" title="分享" id="${templet.uuid }_share" href="javascript:share('${templet.name }','${templet.uuid }')"> <i class="layui-icon layui-icon-share"></i> </a></td>
					</tr>
					<script lang='javascript' type='text/javascript'>
					descr(${st.index });
					</script >
 ...
 <script lang='javascript' type='text/javascript'>
	...
	function add() {
		...
		parent.layer.prompt({
			  ...
			  area: ['800px', '350px'] //自定义文本域宽高
				...
				uri ='${pageContext.request.contextPath}/action_albgenre_templet/add?name='+ value;
	...
	function report(title,id) {
		...
		var uri="${pageContext.request.contextPath}/action_albgenre_templet/integrityCheck/"+id;
		var msg;
		$.ajax({
			url: uri,
			type: 'post',
		...
		parent.layer.confirm(msg+'您确定将“ ' + title + ' ”的模板上传吗？',{title:'提醒'}, function(index){
		  ...
		  uri="${pageContext.request.contextPath}/action_albgenre_templet/report/"+id;
	...
	function def(title,id) {
		...
		parent.layer.confirm('您将“ ' + title + ' ”的模板设为默认吗？',{title:'提醒'}, function(index){
		  ...
		  var uri="${pageContext.request.contextPath}/action_albgenre_templet/def/"+id;
...
function del(title,id) {
	  ...
	  parent.layer.confirm('您确定删除名称为“ ' + title + ' ”的模板吗？<br />删除后将不可恢复！',{title:'提醒'}, function(index){
		  ...
		  var uri="${pageContext.request.contextPath}/action_albgenre_templet/del/"+id;
...
function chgstatus(title,id) {
	  ...
	  parent.layer.confirm('您确定修改名称为“ ' + title + ' ”的可用状态吗？',{title:'提醒'}, function(index){
		  ...
		  var uri="${pageContext.request.contextPath}/action_albgenre_templet/chgstatus/"+id;
...
function market() {
	...
	parent.layer.open({
	    ...
	    title: '云端模板市场-门户区',
	    area : ['1000px' , '700px'],
	    content: '${pageContext.request.contextPath}/action_albgenre_templet/marketlist/',
...
function details(title,id) {
	...
	parent.layer.open({
	    ...
	    title: '模板 '+title+' 内容',
	    area : ['700px' , '500px'],
	    content: '${pageContext.request.contextPath}/action_albgenre_templet/fields/'+id+"/0",
	    success: function(layero, index){
	    },
	    end: function(){
	    	location.reload();
...
function copy(id) {
	parent.layer.prompt({
		...
		if (newTitle!=''){
			var uri="${pageContext.request.contextPath}/action_albgenre_templet/copy/"+id+'?newTitle='+newTitle;
...
layui.use('upload', function(){
  ...
  var uploadInst = upload.render({
    elem: '#test1' //绑定元素
    ,url: '${pageContext.request.contextPath}/action_albgenre_templet/import' //上传接口
    ,accept: 'file'
    ,exts: 'portal'
    ,done: function(res){
    	if (res.code==0){
