...
<div style="padding: 20px; background-color: #F2F2F2;">
  ...
  <tbody>
    ...
    <td colspan="2" align="center" >
		<a href="javascript:poll();"> <i class="layui-icon layui-icon-praise" style="font-size: 26px;vertical-align: middle;"></i> 　(<span id="poll_agrees">{$tag:poll_agrees$}</span>)</a>    
	</td>
    </tr>
...
var poolID;
			...
			$.ajax({
				...
				success: function(data) {
					...
					var marketUrl="${requestScope.marketUrl}";
					var demourl=data.demo;
					if (marketUrl.endWith('/') && demourl.startWith('/') ){
						demourl=demourl.substring(1,demourl.length);
					}
					demourl=marketUrl+demourl;
					$("#demo").attr("src",demourl);
					
					free=data.free;
					$("#poll_agrees").html(data.poll.agrees);
					poolID=data.poll.id;
...
function poll(){
	...
	var w=parentW();
	var uri='${requestScope.marketUrl}/action_poll/poll/'+poolID+'/1';
	//alert(uri);
	$.ajax({
		url:uri,
		type: 'post',
