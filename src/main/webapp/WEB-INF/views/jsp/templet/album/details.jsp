...
layui.use('form', function(){
  ...
  form.on('select(test)', function(data){
		 var url="${pageContext.request.contextPath}/action_album_templet/fields/${requestScope.templetID }/"+$("#area").val();
		 location.href =url;
	 });
  //各种基于事件的操作，下面会有进一步介绍
});
	...
	<table class="layui-table">
				...
				<tbody>
					...
					<tr>
						<td>${st.index+1}</td>
						<td>${vars}</td>
						<td><a href="${pageContext.request.contextPath}/action_album_templet/viewcode/${requestScope.templetID }/${requestScope.area }/${vars}">查看和编辑</a></td>
					</tr>
					</c:forEach>
				</tbody>
