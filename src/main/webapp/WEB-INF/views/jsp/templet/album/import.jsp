...
<body>
<form name="upload" id="upload" action="${pageContext.request.contextPath}/action_album_templet/import" method="post" enctype="multipart/form-data">
	<span class="sub_file"><input type="file" id="file" name="file" class="layui-btn test" lay-data="{accept: 'file',exts: 'album'}" ></span>
	<span class="sub_but"><input type="button" onclick="javascript:importTemp();" value="导入" /></span>
	<p>
	<button class="layui-btn test" lay-data="{url: '${pageContext.request.contextPath}/action_album_templet/import', accept: 'file',exts: 'album'}">上传文件</button>
...
function importTemp() {
	...
	if ($("#file").val()==''){
		...
		$.ajax({
	        url:"${pageContext.request.contextPath}/action_album_templet/import",
	        type:"post",
	        data:form,
	        processData:false,
	        contentType:false,
